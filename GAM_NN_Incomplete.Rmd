...
```{r lm2, lm2_poly, gam2}
lm2 <- lm(Price~Age+Mileage+Fuel_Type+HP+Weight, data = train_data)
lm2_poly <- lm(Price~Age+I(Age^2)+Mileage+I(Mileage^2)+Fuel_Type+HP+Weight+I(Weight^2), data = train_data)
gam2 <- gam(Price~s(Age,df=4)+s(Mileage,df=4)+Fuel_Type+HP+s(Weight,df=4), data = train_data)

lm2_pred <- predict(lm2, test_data)
lm2_poly_pred <- predict(lm2_poly, test_data)
gam2_pred <- predict(gam2, test_data)

accuracy(lm2_pred, test_data$Price)
accuracy(lm2_poly_pred, test_data$Price)
accuracy(gam2_pred, test_data$Price)
```
...
x_train_nn <- cbind.data.frame(train_data$Price/max(train_data$Price), x_train_nn)
colnames(x_train_nn)[1] <- 'St_Price'
...
set.seed(7)
nn2 <- neuralnet(St_Price~Age+Mileage+Fuel_TypeDiesel+Fuel_TypePetrol+HP+Weight, data=x_train_nn, hidden = c(6,4),
                 act.fct = "logistic")
```
...
# Model deployment
nn2_pred <- predict(nn2, x_test_nn)[,1]*max(train_data$Price)
# Test data error
accuracy(nn2_pred, test_data$Price)
```
