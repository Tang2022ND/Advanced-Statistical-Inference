...
```
Store in the urban area have a higher sales
...
plot(total_data$month, total_data$sales)
```
...
```
base on our graph, sales is right skewed and log_sale can fix it
...
```{r Q5}
# Exercise 5 code
lm0 <- lm(log_sales~1, train_data)
lm_full <- lm(log_sales~store_type+bad_weather+promotion+day+month, train_data)
summary(lm_full)
exp(-0.2219231)-1
TSS <- sum((train_data$log_sales-mean(train_data$log_sales))^2)
RSS <- sum((train_data$log_sales-lm0$fitted.values)^2)
1-(RSS/TSS)
```

**Exercise 6** What is the $R^2$ of `lm_full`? What is the impact of `bad_weather` to `log_sales` and thus to `sales`? What is the $R^2$ of `lm0`? Why? Answer:
R^2 for the lm_full is 0.7488, bad weather have a significant impact to the log_sales so is the sales, if their is bad weather, the sale will drop around 20 percent
R^2 for the lm0 is 7.771561e-16
...
```{r}
# Exercise 7 code
lm_bwd <- step(lm_full, k = log(length(train_indices)), direction = "backward")
summary(lm_bwd)$r.sq
```
...
```{r}
# prediction of lm0
pred0 <- exp(predict(lm0, test_data))-1

# prediction of full model
pred_full <- exp(predict(lm_full, test_data))-1

# prediction of backward selected model
pred_bwd <- exp(predict(lm_bwd, test_data))-1
...
We should choose lm_bwd since it has the lowest MAE, and due number of variable, lm_bwd is a relatively simpler model compare with lm_full, the prediction is reasonably accurate, our MAPE is only 16.24 percent
...
# Exercise 10
sample_time <- 1:60
plot(test_data$date[sample_time], test_data$sales[sample_time], xlab='date', ylab='sales')
lines(test_data$date[sample_time], test_data$sales[sample_time], col = "black")
...
```{r Optional Exercise}
lm_full1 <- lm(sales~store_type+bad_weather+promotion+day+month, train_data)
lm_bwd1 <- step(lm_full1, k = log(length(train_indices)), direction = "backward")
pred_full1 <- predict(lm_full1, test_data)
pred_bwd1 <- predict(lm_bwd1, test_data)
accuracy(pred_full1, test_data$sales)
accuracy(pred_bwd1, test_data$sales)
accuracy(pred_full, test_data$sales)
accuracy(pred_bwd, test_data$sales)
```
Yes, the log transformation help lower the MAE.
...
```
the mean still predit better, but pred_opt result are more prepared since mean will give us a better MAE, but profit wise, the pred_opt is better.
